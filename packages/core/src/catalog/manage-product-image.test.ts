...
import { addProductImage, deleteProductImage, setHeroProductImage, setPrimaryProductImage } from "./manage-product-image";
...
describe("addProductImage", () => {
  it("inserta la imagen con la posición y la marca principal si es la primera", async () => {
    ...
    expect(inserted[0]).toMatchObject({ product_id: "product-1", url: "https://cdn/img.jpg", position: 0, is_primary: true, is_hero: true });
  });
});
...
describe("setHeroProductImage", () => {
  it("desmarca todas y marca solo la elegida como hero", async () => {
    const calls: { values: Record<string, unknown>; id?: string }[] = [];
    const client = {
      from() {
        return {
    ...
    await setHeroProductImage(client as never, "product-1", "image-2");

    expect(calls[0]?.values).toMatchObject({ is_hero: false });
    expect(calls[1]).toMatchObject({ values: { is_hero: true }, id: "image-2" });
  });
});
